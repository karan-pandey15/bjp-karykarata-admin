...
import {
  listTemplatesPoster,
  createTemplatePoster,
  updateTemplatePoster,
  deleteTemplatePoster,
} from '../services/templatesPosterApi';
...
const TemplatePosters = () => {
  ...
  const [totalTemplatesPoster, setTotalTemplatesPoster] = useState(0);
  ...
  const loadItems = async () => {
    try {
      setLoading(true);
      const data = await listTemplatesPoster({
        page,
        limit: 12,
        q: searchTerm || undefined,
      });
      setItems(data.templatesPoster || []);
      setTotalPages(data.totalPages || 1);
      setTotalTemplatesPoster(data.totalTemplatesPoster || 0);
    } catch (err) {
      console.error(err);
      setItems([]);
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (imageFile) {
        ...
        if (current) {
          await updateTemplatePoster(current._id, fd, true);
        } else {
          await createTemplatePoster(fd, true);
        }
      } else {
        const payload = {
        ...
        if (current) {
          await updateTemplatePoster(current._id, payload, false);
        } else {
          await createTemplatePoster(payload, false);
        }
      }
  ...
  const handleDelete = async (id) => {
    const ok = await confirm({
      title: 'Delete poster?',
      message: 'Delete this template poster permanently? This cannot be undone.',
      confirmText: 'Delete',
      cancelText: 'Cancel',
      tone: 'danger',
    });
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-stone-500 mt-1">
            Simple image posters (separate from Fabric Templates) · {totalTemplatesPoster} total
          </p>
        </div>
      ...
      <div className="bk-panel p-4 mb-8">
        <SearchInput
          placeholder="Search template posters..."
          value={searchTerm}
          onChange={(e) => {
            setPage(1);
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="aspect-[3/4] bk-panel animate-pulse" />
          ))}
        </div>
      ) : items.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 bk-panel">
          <FileImage size={40} className="text-brand-300 mb-4" />
          <h3 className="font-display text-2xl font-bold text-ink">No template posters found</h3>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {items.map((item) => (
            <div key={item._id} className="bk-panel overflow-hidden group">
              ...
              <div className="p-4">
                <h4 className="font-bold text-ink truncate">{item.templateName}</h4>
          ...
          >
            <div className="p-6 border-b border-brand-50 flex items-center justify-between">
              <h2 className="font-display text-2xl font-bold text-ink">
                {current ? 'Edit Template Poster' : 'Create Template Poster'}
              </h2>
...
export default TemplatePosters;
